...
for file_encoded in $(ls | tr ' ' ''); do
   ...
   time ffmpeg $extra_options -v warning -hide_banner -stats $extra_input_options $extra_input_options -i "$file" -filter:a "volume=10" $extra_output_options -c:v h264_nvenc -preset p1 "$file_new_name"
